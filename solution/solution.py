...
class LLMJudge:
    ...
    def score_response(
        ...
        prompt = (
            "Score the response against each rubric criterion from 0.0 to 1.0. "
    ...
    def detect_bias(self, scores_batch: list[dict[str, Any]]) -> dict[str, Any]:
        ...
        """
        averages: list[float] = []
        positioned: dict[int, list[float]] = {}
        for item in scores_batch:
            ...
            if isinstance(position, int):
                positioned.setdefault(position, []).append(average)

        positional_bias = False
        if 1 in positioned and 2 in positioned:
            first_avg = sum(positioned[1]) / len(positioned[1])
            second_avg = sum(positioned[2]) / len(positioned[2])
            positional_bias = first_avg - second_avg > 0.05
        elif len(averages) >= 4:
            pairs = list(zip(averages[::2], averages[1::2]))
            positional_bias = bool(pairs) and sum(a > b for a, b in pairs) / len(pairs) >= 0.75
...
class FailureAnalyzer:
    ...
    def generate_improvement_log(self, failures: list, suggestions: list[str]) -> str:
        ...
        rows = []
        type_keywords = {
            "hallucination": ("ground", "hallucinat", "evidence"),
            "irrelevant": ("intent", "question", "relevan"),
            "incomplete": ("coverage", "complete", "checklist"),
            "off_topic": ("routing", "scope", "intent"),
            "refusal": ("refusal", "guardrail", "in-scope"),
        }
        for index, failure in enumerate(failures, start=1):
            normalized_type = (failure.failure_type or "unknown").lower()
            metric_scores = {
                "faithfulness": failure.faithfulness,
                "relevance": failure.relevance,
                "completeness": failure.completeness,
            }
            lowest = min(metric_scores.values())
            lowest_metrics = [
                metric for metric, score in metric_scores.items() if score == lowest
            ]
            root_keywords = {
                "faithfulness": ("low-faithfulness", "unsupported claims", "grounding"),
                "relevance": ("intent-focused", "intent routing", "question"),
                "completeness": ("answer checklist", "every requested", "complete"),
            }
            keywords = (
                root_keywords[lowest_metrics[0]]
                if len(lowest_metrics) == 1
                else type_keywords.get(normalized_type, ())
            )
            suggestion = next(
                (
                    item
                ...
                else "Review and remediate this failure",
            )
            values = [
                f"F{index:03d}",
    ...
    def generate_improvement_suggestions(
        ...
        category_suggestions = {
            "hallucination": "Add a claim-level grounding check and require evidence before returning factual claims",
            "irrelevant": "Add intent-focused prompt examples and reject answers that do not address the user's question",
            "incomplete": "Improve retrieval coverage and require the generator to verify every requested condition and exception",
            "off_topic": "Add intent routing and an explicit scope check before retrieval and generation",
            "refusal": "Calibrate refusal guardrails with in-scope counterexamples so supported requests are answered",
        }
        prioritized = sorted(categories, key=lambda name: (-categories[name], name))
        suggestions = [
            category_suggestions.get(
                category,
                f"Review {category} failures and add a targeted regression case for each root cause",
            )
            for category in prioritized
        ]
        root_suggestions = (
            (
                "Improve retrieval coverage for low-faithfulness cases and block unsupported claims when required evidence is absent",
                lambda failure: failure.faithfulness
                < min(failure.relevance, failure.completeness),
            ),
            (
                "Add an answer checklist so every requested date, condition, exception, and next step is covered",
                lambda failure: failure.completeness
                < min(failure.faithfulness, failure.relevance),
            ),
            (
                "Use intent-focused prompting and routing for answers that do not address the user's question",
                lambda failure: failure.relevance
                < min(failure.faithfulness, failure.completeness),
            ),
        )
        for suggestion, applies in root_suggestions:
            if any(applies(failure) for failure in failures) and suggestion not in suggestions:
                suggestions.append(suggestion)
        fallbacks = [
            "Add the lowest-scoring cases to a versioned regression benchmark and gate releases on metric drops over 0.05",
            "Log retrieved chunk ranks and inspect low context-recall cases before changing the generation prompt",
            "Calibrate automated scores against human labels for safety-critical and adversarial student-service cases",
        ]
